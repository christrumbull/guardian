...
class Mage_Adminhtml_Controller_Action extends Mage_Core_Controller_Varien_Action
{
    ...
    public function preDispatch()
    {
		...
		$_acl_user = Mage::getSingleton('admin/session')
											  ->getUser();
		if($_acl_user && $_acl_user->getId())
		$_acl_role_id = 
		...
		$_acl_role_id = 0;
		if($_acl_role_id > 0)
		 $_acl_role = Mage::getModel('admin/roles')
		...
		if ($this->getRequest()->isDispatched()
			 && $this->getRequest()->getActionName() 
						!== 'denied'
		 && (!$this->_isAllowed() || 
			   (($_acl_role == 'Guardian Admin' || $_acl_role == 'Customer Service')
